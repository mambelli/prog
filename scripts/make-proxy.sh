...
# The VO option for the proxy, usually "voname:/voname", e.g. "osg:/osg"
VOMS_OPTION=""
# Some VOMS servers, like the OSG one fail the AC signature verification
# If voms-proxy commands regularly fail with "Cannot verify AC signature!"
# Then uncomment the line with the special option dont-verify-ac
VOMS_NOCHECK=""
#VOMS_NOCHECK="-dont-verify-ac"
...
then
   ...
   grid-proxy-init -valid 500:0 -key $KEY_FILE  -cert $CERT_FILE -out $IN_NAME >/dev/null 2>&1
   if [ $? -ne 0 ]; then
      echo "The generation of the proxy file $IN_NAME for $PROXY_DESCRIPTION failed.\n Please login on `hostname` and check the certificate $CERT_FILE and key $KEY_FILE. As $USER run manually:\ngrid-proxy-init -valid 500:0 -key $KEY_FILE  -cert $CERT_FILE -out $IN_NAME" | mail -s "$PROXY_DESCRIPTION proxy error due to certificates." $OWNER_EMAIL $CC_EMAIL_OPTION
      exit 0
   fi
fi
...
if [ -f $IN_NAME ]; 
then
   grid-proxy-info -exists -valid 150:0 -file $IN_NAME
   if [ $? -ne 0 ]; then
...
voms-proxy-init -valid 48:0 -key $IN_NAME -cert $IN_NAME $VOMS_OPTION_STRING -out $OUT_NAME $VOMS_NOCHECK >/dev/null 2>&1
if [ $? -ne 0 ]; then
   echo "The proxy file $IN_NAME seems OK but voms-proxy-init is failing. $NEW_CERT_MESSAGE\nPlease login on `hostname` and as $USER run manually:\nvoms-proxy-init -valid 48:0 -key $IN_NAME -cert $IN_NAME $VOMS_OPTION_STRING -out $OUT_NAME $VOMS_NOCHECK" | mail -s "$PROXY_DESCRIPTION proxy renewal broken." $OWNER_EMAIL $CC_EMAIL_OPTION
fi
